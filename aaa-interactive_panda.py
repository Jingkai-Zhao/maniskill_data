...
def main(args: Args):
    ...
    num_h5 = len([f for f in os.listdir(output_dir) if f.endswith('.h5')]) if os.path.exists(output_dir) else 0
    
    # 加载轨迹记录
    metadata = {"total_trajectories": 0, "completed_trajectories": []}
    metadata_path = os.path.join(output_dir, "metadata.json")
    if os.path.exists(metadata_path):
        with open(metadata_path, 'r') as f:
            metadata = json.load(f)
    completed_trajectories = metadata.get("completed_trajectories", [])
    start_idx = len(completed_trajectories)
    
    print(f"数据将保存到: {output_dir}")
    if start_idx > 0:
        print(f"将从轨迹索引 {start_idx} 开始继续采集")
        print(f"已完成 {len(completed_trajectories)} 个轨迹")
    ...
    env = RecordEpisode(
        ...
        trajectory_name="trajectory_{}".format(num_h5),
        save_video=False,
        info_on_video=False,
    ...
    while True:
        ...
        code, steps = solve(env, debug=False, vis=True)
        if code == "quit":
            num_trajs += 1
            break
        elif code == "continue":
            ...
            metadata["completed_trajectories"] = completed_trajectories
            metadata["total_trajectories"] = num_trajs+1
...
def solve(env: BaseEnv, debug=False, vis=False):
    ...
    while True:
        ...
        if execute_current_pose:
            ...
            steps.append(planner.elapsed_steps)
            execute_current_pose = False
